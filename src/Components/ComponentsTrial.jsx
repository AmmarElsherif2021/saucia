...
export const ALT = ({ message, type = "success", dismissible = false, icon, timeout, onDismiss }) => {
  ...
  return (
    <Alert
      ...
      variant="solid"
      border={"solid 2px"}
      colorScheme={type === "success" ? "success" : 
                  type === "error" ? "error" : 
    ...
    <Box>
      {[...Array(maxStars)].map((_, i) => {
        ...
        return (
          <StarIcon
            key={starValue}
            color={starValue <= (hoverRating || rating) 
              ? "brand.500" 
              : colorMode === "dark" ? "gray.400" : "gray.200"}
...
// Accordion Component - Using theme colors
export const ACC = ({ sections = [], expandedItem, onToggle }) => {
  return (
  ...
  return (
    <Table variant="simple">
      <Thead>
        <Tr>
          {headers.map((header) => (
            header !== "items" && <Th key={header}>{header}</Th>
          ))}
          {itemsColumnExists && (
            <>
              <Th>Item Name</Th>
        ...
        {data.map((row, rowIndex) => {
          ...
          return items.map((item, itemIndex) => (
            <Tr key={`row-${rowIndex}-item-${itemIndex}`}>
              ...
              {itemIndex === 0 && headers.map((header) => (
                header !== "items" && (
    ...
    <Box p={4} bg={colorMode === "dark" ? "brand.900" : "gray.50"}>
      ...
      <Stack spacing={6}>
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Button</Heading>
          <Flex gap={4} wrap="wrap">
            <BTN onClick={() => alert("Button clicked")}>Default</BTN>
            <BTN variant="outline" onClick={() => alert("Outline clicked")}>Outline</BTN>
            <BTN variant="ghost" onClick={() => alert("Ghost clicked")}>Ghost</BTN>
            <BTN size="xl" onClick={() => alert("Large clicked")}>Large</BTN>
          </Flex>
        </Box>

        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Text Input</Heading>
          <TXT 
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Text Area</Heading>
          <TXTAREA 
            value={textValue} 
            onChange={(e) => setTextValue(e.target.value)} 
          />
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Checkbox</Heading>
          <CHK 
            label="Check me" 
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Radio Buttons</Heading>
          <RDO 
            name="radioGroup" 
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Dropdown</Heading>
          <DD options={dropdownOptions} selectedItem="opt1" onChange={() => {}} />
        </Box>

        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Alert</Heading>
          <ALT 
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Date Picker</Heading>
          <DP selectedDate={selectedDate} onSelect={setSelectedDate} />
        </Box>

        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Slider</Heading>
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Rating</Heading>
          <RATE value={rating} onRateChange={setRating} />
          <Text mt={2}>Rating: {rating} stars</Text>
        </Box>

        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Accordion</Heading>
          <ACC 
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Tabs</Heading>
          <TAB 
            items={tabItems} 
            activeTab={activeTab} 
        ...
        <Box layerStyle={colorMode === "dark" ? "card-dark" : "card"}>
          <Heading size="md" mb={3} textStyle="heading">Loader</Heading>
          <Flex gap={4}>
            <LD />
            <LD type="progress" />
          </Flex>
