...
export const setAdminStatus = async (uid, isAdmin) => {
  try {
    ...
    const res = await fetch(`${import.meta.env.VITE_BASE_URL}/users/set-admin`, {
      method: 'POST',
      headers: {
...
export const getUserInfo = async (uid) => {
  try {
    ...
    const res = await fetch(`${import.meta.env.VITE_BASE_URL}/users/${uid}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${idToken}`,
...
export const getAllUsers = async () => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      return data.users
    } else if (typeof data === 'object' && Object.values(data).every((item) => item?.username)) {
